...
import EDIT_RESERVATION_MUTATION from './editReservation.mutation'
...
const Modal: FC<Props> = ({ isOpen, label, onClose, data, type: reservationType }) => {
  ...
  // Mutations
  const [editMutation] = useMutation(EDIT_RESERVATION_MUTATION)
  ...
  const handleSubmit = async (): Promise<void> => {
    ...
    if (emptyValues) {
      ...
      waitFor(2).then(async () => {
        ...
        const { data: dataCreateReservations } = await editMutation({
          variables
        })
      ...
      <CSS.Modal>
        ...
        <div className="actions">
          ...
          <ModalBtn
            ...
            isLoading={loading}
            loadingText={t('updatingReservation')}
          >
            {t('editReservation')}
          </ModalBtn>
        </div>
