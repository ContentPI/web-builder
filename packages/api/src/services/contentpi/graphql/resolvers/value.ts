import { getEntries } from '@web-builder/utils'
...
export default {
  ...
  Mutation: {
    ...
    updateValues: async (
      _: any,
      { entry, input }: { entry: string; input: any[] },
      { models }: { models: any }
    ): Promise<any> => {
          ...
          const updateValuePromise = new Promise((resolve: any) => {
            models.Value.findAll({
              ...
              if (valueToEdit[0]) {
                ...
                models.Value.create(item).then((createdValue: any) => resolve(createdValue))
              }
            })
          })
    ...
    publishOrUnpublishEntries: async (
      ...
      if (action === 'unpublish') {
        newEntries = entries.filter((entry: any) => entry.status === 'Published')
      }

      newEntries.forEach((entry: any) => {
        const publishedOrUnpublishedValuePromise = new Promise((resolve: any) => {
          models.Value.update(
        ...
        publishedOrUnpublishedValuesPromises.push(publishedOrUnpublishedValuePromise)
      })

      const publishedOrUnpublishedValues = await Promise.all(publishedOrUnpublishedValuesPromises)
